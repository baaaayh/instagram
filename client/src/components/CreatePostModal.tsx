    ...
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          queryClient.invalidateQueries({ queryKey: ["userPageData"] });
        }
      } catch (error) {
        console.log(error);
      }
    },
    [croppedImages, dataURLtoFile, setClosePostModal], //eslint-disable-line
  );
    ...
    <ModalContainer isOpen={isOpenPostModal} closeModal={setClosePostModal}>
      ...
      >
        ...
        {state.step !== 0 && (
          <div className={styles["create-post__header"]}>
            <div className={styles["create-post__row"]}>
              ...
              <h2>{state.step !== 2 ? "자르기" : "새 게시물 만들기"}</h2>
              {state.step !== 2 ? (
                <button
                  type="button"
                ...
                >
                  {filesURL.map((image) => (
                    <CropComponent
                      ...
                      initialZoom={cropInfos[image]?.zoom || 1}
            ...
            <div className={styles["create-post__write"]}>
              <div className={styles["create-post__inset"]}>
                <div className={styles["create-post__profile"]}>
                  <div className={styles["create-post__user"]}>
                    <span className={styles["create-post__img"]}>
                      <ProfileIcon width={24} height={24} />
                    </span>
                    <span className={styles["create-post__nickname"]}>
                      baaaayh
                    </span>
                  </div>
                </div>
                <div className={styles["create-post__textarea"]}>
                  <textarea
                    name="text"
                    id="text"
                    value={textArea || ""}
        ...
        >
          ...
          />
          <input type="hidden" name="textAreaValue" value={textArea} />
