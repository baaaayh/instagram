...
export default memo(function UserComponent({
  ...
  const handleFollow = useCallback(() => {
    recommendUsersMutation.mutate(undefined, {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ["feedList"] });
      },
    });
  }, [recommendUsersMutation, queryClient]);
      ...
      <div className={styles["user__inner"]}>
        <div className={styles["user__profile"]}>
          <div className={styles["user__figure"]}>
            <Link to={`/${nickName}`} onClick={() => setHistory(data)}>
              {profile_image ? (
                <img src={profile_image} alt={`${nickName}의 프로필`} />
              ) : (
