...
export default memo(function FeedCommentInput({
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      feedDataMutation.mutate(undefined, {
        onSuccess: () => {
          queryClient.invalidateQueries({
            queryKey: ["feedList", userNickName],
          });
    ...
    [feedDataMutation], //eslint-disable-line
  );
    ...
    <div className={styles["comment-input"]}>
      <form onSubmit={handleSubmit}>
        <div className={styles["comment-input__row"]}>
          ...
          <input type="hidden" name="user_nickname" value={userNickName} />
          <input
            type="hidden"
          ...
          />
          {comment.length > 0 && (
            <button type="submit" className={styles["comment-input__button"]}>
              <span>게시</span>
            </button>
          )}
        </div>
